<x-dynamic-component :component="$getFieldWrapperView()" :id="$getId()" :label="$getLabel()" :label-sr-only="$isLabelHidden()" :helper-text="$getHelperText()"
    ...
    <div>
        @if (count($containers) > 1 && $isCollapsible)
            <div class="space-x-2 rtl:space-x-reverse" x-data="{}">
                <x-forms::link x-on:click="$dispatch('repeater-collapse', '{{ $getStatePath() }}')" tag="button"
                    size="sm">
                    {{ __('forms::components.repeater.buttons.collapse_all.label') }}
                </x-forms::link>

                <x-forms::link x-on:click="$dispatch('repeater-expand', '{{ $getStatePath() }}')" tag="button"
                    size="sm">
    ...
    <div
        ...
        @if (count($containers))
            <ul>
                <x-filament-support::grid :default="$getGridColumns('default')" :sm="$getGridColumns('sm')" :md="$getGridColumns('md')" :lg="$getGridColumns('lg')"
                    :xl="$getGridColumns('xl')" :two-xl="$getGridColumns('2xl')" wire:sortable
                            ...
                            @if (!$isItemMovementDisabled || !$isItemDeletionDisabled || $isCloneable || $isCollapsible || $hasItemLabels)
                                <header
                                    ...
                                    @unless ($isItemMovementDisabled)
                                        <button title="{{ __('forms::components.repeater.buttons.move_item.label') }}"
                                            ...
                                            <x-heroicon-s-switch-vertical class="h-4 w-4" />
                                        </button>
                                    @endunless
                                    ...
                                    <ul @class([
                                            ...
                                            @unless ($loop->first)
                                                <li>
                                                    <button
                                                        ...
                                                        <x-heroicon-s-chevron-up class="h-4 w-4" wire:loading.remove.delay
                                                            wire:target="dispatchFormEvent('repeater::moveItemUp', '{{ $getStatePath() }}', '{{ $uuid }}')" />

                                                        <x-filament-support::loading-indicator
                                                            class="h-4 w-4 text-primary-500" wire:loading.delay
                                                            wire:target="dispatchFormEvent('repeater::moveItemUp', '{{ $getStatePath() }}', '{{ $uuid }}')"
                                                            x-cloak />
                                                    </button>
                                                </li>
                                            ...
                                            @unless ($loop->last)
                                                <li>
                                                    <button
                                                        ...
                                                        <x-heroicon-s-chevron-down class="h-4 w-4" wire:loading.remove.delay
                                                            wire:target="dispatchFormEvent('repeater::moveItemDown', '{{ $getStatePath() }}', '{{ $uuid }}')" />
                                        ...
                                        @if ($isCloneable)
                                            <li>
                                                <button
                                                    ...
                                                    <x-heroicon-s-duplicate class="h-4 w-4" wire:loading.remove.delay
                                                        wire:target="dispatchFormEvent('repeater::cloneItem', '{{ $getStatePath() }}', '{{ $uuid }}')" />
                                        ...
                                        @unless ($isItemDeletionDisabled)
                                            <li>
                                                <button
                                                    onclick="confirm('Êtes-vous sûr de vouloir le supprimer ?') || event.stopImmediatePropagation()"
                                                    ...
                                                    <x-heroicon-s-trash class="h-4 w-4" wire:loading.remove.delay
                                                        wire:target="dispatchFormEvent('repeater::deleteItem', '{{ $getStatePath() }}', '{{ $uuid }}')" />
                                        ...
                                        @if ($isCollapsible)
                                            <li>
                                                <button
                                                    ...
                                                    <x-heroicon-s-minus-sm class="h-4 w-4" x-show="! isCollapsed" />

                                                    <span class="sr-only" x-show="! isCollapsed">
                                                        {{ __('forms::components.repeater.buttons.collapse_item.label') }}
                                                    </span>

                                                    <x-heroicon-s-plus-sm class="h-4 w-4" x-show="isCollapsed"
                                                        x-cloak />
